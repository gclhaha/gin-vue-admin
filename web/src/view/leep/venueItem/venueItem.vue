  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateVenueItemFunc(scope.row)">变更</el-button>
       ...
       <template #header>
          ...
          <el-form :model="formData" label-position="top" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="场地名称:"  prop="name" >
              <el-input v-model="formData.name" :clearable="true"  placeholder="请输入场地名称" />
            </el-form-item>
          </el-form>
    </el-drawer>
    ...
    <el-drawer destroy-on-close size="800" v-model="detailShow" :show-close="true" :before-close="closeDetailShow">
            <el-descriptions :column="1" border>
                    <el-descriptions-item label="所属场馆ID">
                        {{ detailFrom.venueId }}
...
const getTableData = async() => {
  const table = await getVenueItemList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteVenueItemByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVenueItemFunc = async(row) => {
    const res = await findVenueItem({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteVenueItemFunc = async (row) => {
    const res = await deleteVenueItem({ id: row.id })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createVenueItem(formData.value)
                  break
                case 'update':
                  res = await updateVenueItem(formData.value)
                  break
                default:
                  res = await createVenueItem(formData.value)
                  break
