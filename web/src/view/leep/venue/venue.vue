  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column type="selection" width="55" />
          ...
          <el-table-column align="left" label="场馆名称" prop="name" width="120" />
          <el-table-column align="left" label="省" prop="province" width="120" />
          <el-table-column align="left" label="市" prop="city" width="120" />
          <el-table-column align="left" label="区/县" prop="district" width="120" />
        ...
        <el-table-column align="left" label="操作" fixed="right" min-width="240">
            ...
            <el-button  type="primary" link icon="edit" class="table-button" @click="updateVenueFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getVenueList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
  ElMessageBox.confirm('确定要删除吗?', '提示', {
    ...
    type: 'warning'
      ...
      const res = await deleteVenueByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateVenueFunc = async(row) => {
    const res = await findVenue({ id: row.id })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteVenueFunc = async (row) => {
